# Hi! 👋🏻
My name is Matvey, and I am a programmer from 🇷🇺 Russia.

# Programming Languages
* JavaScript
* C

# Projects
* [winerr](https://github.com/shikoshib/winerr) - A fast and accurate Windows fake error messages generator that works directly in a browser
* [gj-boomlings-api](https://github.com/shikoshib/gj-boomlings-api) - A light-weight Geometry Dash API wrapper
* [3xSummoner](https://github.com/shikoshib/3xSummoner) - A message box generator that uses native Windows API, intended for use on Windows 3.1 (literally my first C program ever)

---
Discord: `shikoshib`
